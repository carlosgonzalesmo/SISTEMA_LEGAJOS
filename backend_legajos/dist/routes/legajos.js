...
router.get('/by-codigo/:codigo', auth_1.authMiddleware, denySysadmin, async (req, res, next) => {
    try {
        ...
        const legajo = await prisma_1.prisma.legajo.findFirst({ where: { codigo: padded }, include: { usuario: true, archivos: true } });
        if (!legajo)
...
const createLegajoSchema = zod_1.z.object({
    codigo: zod_1.z.string().min(3), // e.g. L-5 or L-0005
    titulo: zod_1.z.string().min(2), // nombre descriptivo
...
router.post('/', auth_1.authMiddleware, denySysadmin, async (req, res, next) => {
    try {
        ...
        if (isNaN(numero) || numero < 0)
            return res.status(400).json({ error: 'Número inválido en código' });
...
router.put('/:id', auth_1.authMiddleware, denySysadmin, async (req, res, next) => {
    try {
        ...
        if (req.body.codigo) {
            ...
            const padded = numero.toString().padStart(4, '0');
            const finalCodigo = `${letra}-${padded}`;
            const exists = await prisma_1.prisma.legajo.findFirst({ where: { codigo: finalCodigo, NOT: { id } } });
            if (exists)
                return res.status(409).json({ error: 'Código ya existe' });
