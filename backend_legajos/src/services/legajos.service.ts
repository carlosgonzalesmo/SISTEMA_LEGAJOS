...
export const LegajosService = {
  ...
  create: (data: { codigo: string; titulo: string; descripcion?: string; dniCe?: string; usuarioId: number; estado: string }) => prisma.legajo.create({ data }),
  update: (id: number, data: Partial<{ codigo: string; titulo: string; descripcion?: string; dniCe?: string; estado: string }>) => prisma.legajo.update({ where: { id }, data }),
