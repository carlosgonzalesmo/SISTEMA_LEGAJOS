...
export function paddedCodigo(raw: string): string {
...
export function validateCodigo(raw: string): { valid: boolean; error?: string; normalized?: string; padded?: string } {
  const normalized = normalizeCodigo(raw);
  const m = normalized.match(/^([A-Z])-(\d+)$/);
...
export function validateDniCe(value: string | undefined | null): { valid: boolean; error?: string } {
  if (!value) return { valid: true };// optional field
  const v = value.trim();
  if (/^\d{8}$/.test(v) || /^\d{12}$/.test(v)) return { valid: true };
